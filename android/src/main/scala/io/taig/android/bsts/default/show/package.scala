...
import io.taig.bsts.{ Rule, Report$ }

package object show {
    private def show[R <: Rule]( id: Int, args: Any* )( implicit context: Context ) = {
        Report[R]( context.getString( id ).format( args: _* ) )
    }

    implicit def `Show[Email]`( implicit context: Context ) = show[Email]( R.string.bsts_email )

    implicit def `Show[Exactly]`[T]( implicit context: Context ) = Report[Exactly[T]]((value: T, rule: Exactly[T] ) ⇒ {
        context.getString( R.string.bsts_exactly ).format( rule.length )
    } )

    implicit def `Show[Max]`[T]( implicit context: Context ) = Report[Max[T]]((value: T, rule: Max[T] ) ⇒ {
        context.getString( R.string.bsts_max ).format( rule.length )
    } )

    implicit def `Show[Matches]`[T]( implicit context: Context ) = Report[Matches[T]]((value: T, rule: Matches[T] ) ⇒ {
        rule.target match {
    ...
    implicit def `Show[Min]`[T]( implicit context: Context ) = Report[Min[T]]((value: T, rule: Min[T] ) ⇒ {
